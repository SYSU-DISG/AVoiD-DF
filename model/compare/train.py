...
def main(args):
    ...
    train_images_path, train_images_label, train_audio_path, train_audio_label, val_images_path, val_images_label, val_audio_path, val_audio_label = read_split_data(
        args.faces_data_path, args.audio_data_path)
    ...
    cast_pg = [p for p in castmodel.parameters() if p.requires_grad]
    optimizer1 = optim.SGD(cast_pg, lr=args.lr, momentum=0.9, weight_decay=5E-5)
    optimizer2 = optim.SGD(pg, lr=args.lr, momentum=0.9, weight_decay=5E-5)
    ...
    for epoch in range(args.epochs):
        # train
        train_loss, train_acc = train_one_epoch(castModel=castmodel,
                                                model=model,
        ...
        # validate
        val_loss, val_acc = evaluate(castModel=castmodel,
                                     model=model,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--faces_data-path', type=str,
                        default="./faces")
